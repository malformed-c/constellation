...
The real fix is that a Kubernetes control plane can legitimately bind several interfaces at once (e.g. loopback on the colocated node, plus one or more shared LAN addresses reachable from every node) — so the correct model isn't "detect which one address this node can reach," it's "give the agent every valid candidate and let it pick." Cilium's `pkg/k8s/client` already has exactly this built in: `--k8s-api-server-urls` (`k8sAPIServerURLs` in `values.yaml`, a list) feeds `restConfigManager`, which retries other candidates in a tight loop on connection failure (no shell-script sleep/poll needed) and keeps re-verifying `/readyz` at runtime via a heartbeat controller. **This fork makes the list an ordered preference list** — first entry at startup, later ones as fallbacks in order (`selectPreferredAPIServerURL`, and `rotateAPIServerURL` advancing by index). Upstream picks at random, which spreads load across *distinct* API servers; here the entries are usually several network paths to the *same* control plane, so there is nothing to spread and random selection meant an arbitrary path per agent start — with a measured cost, since agents landing on the wireless address were implicated in leader-election churn on this cluster. The trade-off, so it is not rediscovered: on a genuine multi-API-server deployment every agent now prefers the same first entry rather than spreading — see `pkg/k8s/client/rest_config_provider.go` and `cell.go`'s `waitForConn`/`startHeartbeat`. Set `k8sAPIServerURLs` to every address the control plane binds; leave it empty (default) for a single-address deployment, which falls straight through to the plain env vars unchanged.
