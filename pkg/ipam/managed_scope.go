...
func (m *managedScopeAllocator) addCIDR(nodeName string, cidr *net.IPNet) {
	prefix, ok := netipx.FromStdIPNet(cidr)
	if !ok {
		m.logger.Warn("Managed IPAM: invalid CIDR, skipping sub-allocator",
	...
	m.logger.Info("Managed IPAM: added sub-allocator",
		logfields.NodeName, nodeName,
		logfields.V4Prefix, prefix.String())
}
...
func (m *managedScopeAllocator) AddCIDR(nodeName string, cidr *net.IPNet) {
	...
	prefix, ok := netipx.FromStdIPNet(cidr)
	if !ok {
		m.logger.Warn("Managed IPAM: invalid CIDR, ignoring",
			logfields.NodeName, nodeName,
			logfields.V4Prefix, cidr.String())
		return
	}
...
// findSubForIP returns the sub-allocator whose CIDR contains addr.
func (m *managedScopeAllocator) findSubForIP(addr netip.Addr) *subAllocator {
	for _, sub := range m.subs {
		if sub.allocCIDR.Contains(addr) {
			return sub
		}
	}
	return nil
...
func (m *managedScopeAllocator) Allocate(addr netip.Addr, owner string, pool Pool) (*AllocationResult, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	sub := m.findSubForIP(addr)
	if sub == nil {
		return nil, fmt.Errorf("IP %s does not belong to any managed CIDR", addr)
	}

	if err := sub.allocator.Allocate(addr); err != nil {
		return nil, err
	}
	return &AllocationResult{IP: addr, IPPoolName: Pool(sub.nodeName)}, nil
}

// AllocateWithoutSyncUpstream is identical to Allocate for host-scope.
func (m *managedScopeAllocator) AllocateWithoutSyncUpstream(addr netip.Addr, owner string, pool Pool) (*AllocationResult, error) {
	return m.Allocate(addr, owner, pool)
}

// Release releases an IP back to the sub-allocator owning the CIDR.
func (m *managedScopeAllocator) Release(addr netip.Addr, pool Pool) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	sub := m.findSubForIP(addr)
	if sub == nil {
		return fmt.Errorf("IP %s does not belong to any managed CIDR", addr)
	}

	sub.allocator.Release(addr)
	return nil
}
...
func (m *managedScopeAllocator) AllocateNext(owner string, pool Pool) (*AllocationResult, error) {
	...
	addr, err := sub.allocator.AllocateNext()
	if err != nil {
...
func (m *managedScopeAllocator) Dump() (map[Pool]map[string]string, string) {
	...
	for _, sub := range m.subs {
		sub.allocator.ForEach(func(addr netip.Addr) {
			alloc[addr.String()] = ""
		})
		totalCapacity += ip.CountIPsInCIDR(netipx.PrefixIPNet(sub.allocCIDR)).Uint64()
	}
...
func (m *managedScopeAllocator) Capacity() uint64 {
	...
	var total uint64
	for _, sub := range m.subs {
		total += ip.CountIPsInCIDR(netipx.PrefixIPNet(sub.allocCIDR)).Uint64()
	}
	return total
}
...
func ciliumNodeIPv4CIDR(cn *ciliumv2.CiliumNode) *net.IPNet {
	for _, prefix := range cn.Spec.IPAM.PodCIDRs {
		if prefix.Addr().Is4() {
			return netipx.PrefixIPNet(prefix.Prefix)
		}
	}
	return nil
}
