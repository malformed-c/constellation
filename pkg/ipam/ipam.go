		...
		if ipam.config.IPv4Enabled() {
			if useManaged {
				...
				ipam.ipv4Allocator = newManagedScopeAllocator(
					...
					ipam.logger,
					ipam.clientset,
					netipx.PrefixIPNet(ipam.nodeAddressing.IPv4().AllocationCIDR()),
				)
				...
				if !prefix.IsValid() {
					// Upstream changed this path from logging.Fatal to a
					// returned error during the 2026-08 rebase; match it.
